...
def nn_model():
    ...
    model.add(Dense(420, input_dim = xtrain.shape[1], init = 'he_normal'))
    model.add(PReLU())
    model.add(BatchNormalization())
    model.add(Dropout(0.4))
        
    model.add(Dense(210, init = 'he_normal'))
    model.add(PReLU())
    model.add(BatchNormalization())    
    model.add(Dropout(0.2))
    
    model.add(Dense(53, init = 'he_normal'))
    model.add(PReLU())
    model.add(BatchNormalization())    
    model.add(Dropout(0.2))
...
print('Total - AE:', mean_absolute_error(np.exp(y)-200, pred_oob))
...
df.to_csv('submission_keras_shift_perm_big.csv', index = False)
